...
// nvcc Convergance-Testting.cu -o program.out && ./program.out
...
float endtime = (365)*24;   //Days in hours 280;//
...
float D = 60*60*6*powf(10,-12); //Diffusion rate at 37 degrees celsius unit: m^2/h
...
//    strcat(Path_to_Folder,"_");
    ...
    sprintf(Buffer,"%.6f",timestep);
    strcat(Path_to_Folder,Buffer);
    strcat(Path_to_Folder,"-");
    strcat(Path_to_Folder,"timestep");
...
void initailConditions(int Nx, int Ny){
    ...
    if (INITIALVIRUS == 1){
        vtemp[(NumberOfLayers-1)+Nx*(NumberOfLayers-1)+Nx*Ny*0] = powf(10,12);
        vtemp[(NumberOfLayers-1)+Nx*(NumberOfLayers-1)+Nx*Ny*1] = powf(10,12);
    }
}
...
void printToFileCellAndVirusAnalysisInitial(int Nx, int Ny){
    ...
    fprintf(outfile9,"%.1f, %d, %d, %d, %d, %f,", 0.0, NumberHealthy1, NumberEclipse1, NumberInfected1, NumberDead1, AmountOfVirus);
    fprintf(outfile9,"\n");
...
void printToFileCellAndVirusAnalysis(float timestep){
    ...
    fprintf(outfile8,"%.1f, %d, %d, %d, %d, %f,", timestep+1.0, NumberHealthy1, NumberEclipse1, NumberInfected1, NumberDead1, AmountOfVirus);
    fprintf(outfile8,"\n");
